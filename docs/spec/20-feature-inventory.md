...
- **CHAT-2 (MUST).** Linked URLs and uploaded files MUST be made available to the model as context for the reply. For the turn a file is attached the model gets it in full (the image itself for a vision-capable model, the extracted text otherwise). For **later turns** the file is **retained inline up to an operator-set token budget**, so a follow-up needs no re-fetch: images are always retained (their cost is bounded and there is no other way to re-*see* one), and a document's text is retained until it exceeds the budget — past which the text is cut off and a pointer to the file-access tools is appended in its place. Beyond that inline window (and for the original bytes of *any* attachment, for processing), the file remains *available to reference* on demand — through the knowledge base (`corpus.retrieve` over its extracted text) and an explicit **provision-into-the-sandbox** action that stages the file's bytes into the agent's workspace — rather than being re-injected into every prompt. The inline budget is operator-configurable.
...
- **CHAT-8 (MUST — mid-run steering).** The user MUST be able to send a further text message in the same conversation while a reply is still being generated. The message is **queued** and handed to the model at the next boundary between model requests — never interrupting an in-flight response stream — and if the run finishes with messages still queued, the system continues the turn with them rather than dropping them. A queued message MUST be visible as pending, editable and withdrawable until it is consumed, and once consumed MUST persist as an ordinary user message in the conversation. Steering is text-only: a message with attachments still requires the run to finish first. A message left queued when a run is cancelled or fails MUST be returned to the user (e.g. restored to the input), never silently discarded.
- **CHAT-9 (SHOULD).** For a request that takes several steps, the agent SHOULD maintain a **task list** for the conversation — each task carrying its own state (pending, in progress, done, cancelled) — and the user MUST be able to see it and watch it change as the work proceeds. The list belongs to the conversation, not to a single reply: it MUST survive a page reload, a disconnect-and-resume, and a turn that pauses for approval. It is the agent's own account of its work, so it is **read-only to the user** — an edit the model could not see would misrepresent what it is actually doing.
