...
- **AE-3.1 (MUST).** A defined set of tools perform **sensitive actions** — running code or commands **on the host**, host filesystem writes, sending email, model download/serve/stop, endpoint/integration/webhook/token/settings configuration, and vault access. The agent MUST NOT complete a sensitive action without the operator's explicit approval of that specific action, presented with what it will do — the action and its concrete arguments — before it takes effect, **except** where a standing pre-authorization already covers it: a scheduling-time grant (`AE-3.5`) or an operator-granted conversation-scoped auto-approval (`AE-3.7`). (**Sandboxed** code execution is deliberately *excluded* from this set: isolated from the host and run only on copies per `XC-SEC-7`, it carries no host-level risk and needs no approval.)
...
- **AE-3.7 (MAY).** When approving an action that paused for approval, the operator MAY additionally grant a **conversation-scoped auto-approval** for that tool, so the *same* tool's later calls **in that conversation** proceed without re-prompting. The grant is **opt-in** (the operator must choose it; the default for every approval remains a single, this-call-only decision), **time-limited** (it lapses after a bounded window), and **revocable** at any time; the operator MUST be able to see which tools are currently auto-approved for a conversation and revoke any of them, and a revoked or lapsed grant returns that tool to per-call approval. The grant is confined to its one conversation and never becomes a global standing approval. This is the interactive analogue of the scheduling-time pre-authorization (`AE-3.5`).
- **AE-3.8 (MUST).** **Global knowledge-base recall** — the agent searching the operator's whole corpus (memory, past conversations, indexed folders/uploads/documents) by relevance — is **approval-gated**, so the operator can keep irrelevant retrieved content out of the model's context: a denial means the content is never read into context. This is a **context-control** gate, distinct from the sensitive-action set of `AE-3.1` (recall is read-only and has no host effect); it is subject to the same conversation-scoped auto-approval (`AE-3.7`). Reading a source the operator has **explicitly referenced** — e.g. a file just attached to the turn — is *not* gated, since the operator already chose to provide it.
