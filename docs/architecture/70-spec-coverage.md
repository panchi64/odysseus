...
| VAULT-1 password vault (secrets manager) | ✅ | `models/secret.py`, `services/secret_vault.py`, `routes/secret_vault.py` (`/vault/*`) | Distinct from the at-rest encryption vault (`core/vault`, `XC-SEC-3`) and independently locked: its own passphrase, its own Argon2id salt, its own key — the login password opens nothing here. The unlocked state is a memory-only session (key + idle deadline), never a column, so a restart re-locks by construction; an app-level lock tears every vault session down with it. Entries are **double-sealed** — under the vault key, then the at-rest DEK — so both locks must be open to read one. |
