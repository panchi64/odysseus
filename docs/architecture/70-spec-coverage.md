...
| DOC-3 AI rewrite / targeted edit / reviewable suggestions | 🚧 | `models/document_suggestion` | **Schema landed.** Full rewrite and targeted edit were already real (`document_create`/`document_edit` stream `document.created`/`delta`/`committed` into the View, each write minting a version). The missing half — a *proposed* change that has **not** been applied — now has its own rows: a suggestion **set** (one AI pass) of independently acceptable **changes**, each a vault-sealed anchored span carrying its own pending/accepted/rejected status. Lifecycle, tool, REST, and the review UI still to come. |
