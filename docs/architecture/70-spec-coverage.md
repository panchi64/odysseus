...
| COOK-3…5 download/serve/manage models | ⬜ | — | Registry handles *endpoint* config, not local serving. Agent download/serve/stop is approval-gated when built. Builds on the surviving hardware-detection package; the hardware-fit leaderboard (UI + backend) was removed (see COOK-1). |
