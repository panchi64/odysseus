...
| XC-SEC-5 untrusted external content marked as data | 🟡 | `core/untrusted.py` (`wrap_untrusted` + `untrusted_preamble`/`untrusted_fence`); applied in `services/webfetch` to fetched pages + `services/search` to search snippets | Marking built and applied at the first ingester (web): sentinel-delimited block + standing "treat as data" instruction. Search decomposes it — one preamble per call, each snippet a bare fence sharing the call's nonce — so a batch doesn't repeat the instruction per result. `ReinjectSystemPrompt` (poisoned-history defense) still pending; extends to uploads/mail as they land. |
