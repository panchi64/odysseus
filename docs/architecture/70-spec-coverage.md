...
| CHAT-2 links/files as model context | ✅ | `agent/attachments`, `agent/engine`, `services/corpus/uploads`, `tools/corpus.py` | Attached files reach the model for the turn they're sent (image-as-pixels for a vision model, extracted text otherwise — `wrap_untrusted`, `XC-SEC-5`); persisted history keeps only a marker, so later turns and other chats pull the file from the corpus via `corpus.retrieve` instead of re-injecting it (available-to-reference, not info-dumped). |
