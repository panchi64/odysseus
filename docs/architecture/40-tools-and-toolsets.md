...
| `browse` | the harness's eighteen Playwright tools (`navigate`, `snapshot`, `click`, `type_text`, `screenshot`, `tabs`, …) | `services/browser` — a conversation-scoped session on a visible Chromium the operator shares (D78), launched on the host behind an SSRF proxy and driven over CDP (see D57) | ✅ built |
