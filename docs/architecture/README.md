...
Deep research is **not** a different skeleton. It was once a rounds-based pipeline of its own — plan, search, read, analyze, write, bounded by rounds and time — and is now a *mode a conversation is in*: the same flow above, with a prompt that says gather before you conclude and a higher round-trip ceiling. The fan-out the pipeline used to own is back, but as ordinary turns: a research thread clarifies the question, submits a plan for the operator to approve, and then launches a `researcher` sub-agent per topic (`subagents_launch`) — each an ordinary turn in a hidden conversation, reporting back to wake the thread that launched it. The rounds loop's concurrency is now the lane's, and its "stop when comprehensive" is the model's own, checked against `subagents_list`.
