...
| Set | License | Author |
|---|---|---|
| [Iconoir](https://github.com/iconoir-icons/iconoir) | MIT | Luca Burgio & contributors |

Most entries in the frontend's icon registry (`frontend/src/ui/icons/registry.tsx`)
are Iconoir geometry, inlined as raw SVG bodies by `frontend/scripts/gen-iconoir.ts`.
Iconoir is not a dependency — the generator fetches the set on demand and only the
extracted markup is committed — so this is the attribution of record.

## Fonts

All self-hosted in `frontend/src/ui/theme/fonts/` — no font is fetched from a
third party at runtime, and none is an npm dependency. The mono faces are
regenerated by `bun run build:fonts`; the display face's woff2 subsets were
lifted from `@fontsource/oswald` v5.3.0 once and committed. License texts sit
beside the fonts (`OFL.txt`, `OFL-Oswald.txt`).

| Font | License | Author |
|---|---|---|
| [JetBrains Mono](https://github.com/JetBrains/JetBrainsMono) — Nerd Font build, subset to woff2 | SIL Open Font License 1.1 | JetBrains |
| [Nerd Fonts](https://github.com/ryanoasis/nerd-fonts) — Braille Patterns patch | SIL Open Font License 1.1 | Ryan Laintis & contributors |
| [Oswald](https://fonts.google.com/specimen/Oswald) — 700 weight, latin + latin-ext woff2 | SIL Open Font License 1.1 | Vernon Adams |
