...
Self-hosted in `frontend/src/ui/theme/fonts/` (mono, regenerated by `bun run build:fonts`) and via `@fontsource` (display):
...
| [Oswald](https://fonts.google.com/specimen/Oswald) (via `@fontsource/oswald`) | SIL Open Font License 1.1 | Vernon Adams |
