 ...
 *  lists. Native `<select>` can't host a filter field, so this is a popover
 *  combobox built from the system primitives. Closes on pick, backdrop, or Esc. */
export function Combobox(props: ComboboxProps): JSX.Element {
  ...
  const filtered = createMemo(() => {
    ...
    if (!q) return props.groups.filter((g) => g.options.length > 0);
    return props.groups
      .map((g) => ({
        label: g.label,
    ...
    <div class={cx("relative inline-flex", props.class)}>
      ...
      <Show when={open()}>
        ...
        <div
          role="listbox"
          class={cx(
            "absolute top-full z-50 mt-1 flex max-h-80 w-64 flex-col border border-line bg-surface",
