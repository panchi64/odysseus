"""`_park_for_approval`/`_approval_conversation_title` in isolation: title wording,
...
from agent.engine import ParkedTurn, _park_for_approval
...
async def test_park_excludes_grant_covered_tool_names(tmp_path):
    ...
    await _park_for_approval(
        ...
        _FakeRequests(_APPROVALS),
        set(),
        pre_approved={"call-1": ToolApproved()},
        notifications=service,
        store=None,
        conversation_id="c1",
    )
...
async def test_park_skips_notifying_when_every_call_is_grant_covered(tmp_path):
    ...
    await _park_for_approval(
        ...
        _FakeRequests(_APPROVALS),
        set(),
        pre_approved={"call-1": ToolApproved(), "call-2": ToolApproved()},
        notifications=service,
        store=None,
        conversation_id="c1",
    )
