...
from tools.shell import _toolset_for
...
def test_the_shell_tools_env_keeps_the_pins_and_still_drops_the_keys(monkeypatch):
    """The two controls compose, which is the whole reason an explicit env is safe here.

    Handing the harness an `env` replaces inheritance, so the question is whether
    `denied_env_patterns` — the thing keeping the operator's model keys out of every
    spawned process — still applies over it. It does: the harness filters whichever base
    it is given. Read through its own resolver, since that is the code that decides.
    """
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-secret")
    toolset = _toolset_for(Path("/nonexistent-template-root"))
    env = toolset._resolve_env()  # noqa: SLF001 - the claim under test is the harness's
