...
The stub *surface* adapters (uploads, gallery, documents, research) exist so the
``/rag`` source list shows every planned surface from day one, but they hold no content
yet (extraction pipelines are deferred): ``retrieve`` returns nothing and ``status``
reports a stale, empty source. Each fills in as its surface is built.
"""
...
class StubSurfaceAdapter(SourceAdapter):
    ...
    Uploads, gallery, documents, and research each get one of these until their
    extraction pipeline lands (deferred). ``retrieve`` returns nothing; ``status``
    reports a stale, empty source so the ``/rag`` list shows it as awaiting build-out.
    """
...
def default_surface_stubs() -> list[StubSurfaceAdapter]:
    ...
    return [
        StubSurfaceAdapter("surf-documents", "Documents", "file", "/documents"),
